...
import { Search, Filter, UtensilsCrossed, ChevronDown } from "lucide-react";
...
import type { Category, MenuItem, Addon, CartItem } from "@shared/schema";
import { motion } from "framer-motion";

export default function MenuPage() {
  const [, navigate] = useLocation();
  const searchParams = useSearch();
  const { tableNumber, tableId, sessionId, setTableInfo } = useSession();
  const { addItem } = useCart();
  const { toast } = useToast();
  
  const [searchQuery, setSearchQuery] = useState("");
  const [selectedCategory, setSelectedCategory] = useState<string>("all");
  const [filterType, setFilterType] = useState<"all" | "vegetarian" | "vegan" | "spicy">("all");

  // Parse URL params for table info
  useEffect(() => {
        ...
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="font-bold text-lg">DineFlow</h1>
                {tableNumber && (
          ...
          <div className="mt-4 flex gap-2">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                ...
                <DropdownMenuItem onClick={() => setFilterType("vegetarian")}>
                  Vegetarian
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => setFilterType("vegan")}>
                  Vegan
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => setFilterType("spicy")}>
                  Spicy
                </DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            {filteredItems.map((item) => (
              <MenuItemCard
                key={item.id}
                item={item}
                addons={getAddonsForItem(item.id)}
