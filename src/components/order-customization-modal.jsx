...
export function OrderCustomizationModal({ item, addons, open, onClose, onAdd }) {
        ...
        <div className="space-y-6 py-4">
          <div className="space-y-2">
            ...
            <div className="flex items-center gap-4">
              <Button size="icon" variant="outline" onClick={() => setQuantity(Math.max(1, quantity - 1))} disabled={quantity <= 1} data-testid="button-decrease-quantity">
                <Minus className="w-4 h-4" />
              </Button>
              <span className="text-xl font-semibold w-8 text-center" data-testid="text-quantity">
                {quantity}
              </span>
              <Button size="icon" variant="outline" onClick={() => setQuantity(quantity + 1)} data-testid="button-increase-quantity">
                ...
                {spiceLevels.map((level) => (
                  <button
                    ...
                    className={cn(
                      ...
                      spiceLevel === level.value ? "border-primary bg-primary/10 text-primary" : "border-border hover:border-muted-foreground"
                    )}
                ...
                {addons.map((addon) => (
                  <div
                    ...
                    className={cn(
                      ...
                      selectedAddons.find((a) => a.id === addon.id) ? "border-primary bg-primary/10" : "border-border hover:border-muted-foreground"
                    )}
                    onClick={() => addon.isAvailable && handleAddonToggle(addon)}
                    data-testid={`addon-${addon.id}`}
                  >
                    <div className="flex items-center gap-3">
                      <Checkbox checked={!!selectedAddons.find((a) => a.id === addon.id)} disabled={!addon.isAvailable} />
                      <span className={cn(!addon.isAvailable && "text-muted-foreground")}>{addon.name}</span>
                    </div>
                    <span className="text-primary font-medium">+${parseFloat(addon.price).toFixed(2)}</span>
                  </div>
                ))}
              </div>
            </div>
