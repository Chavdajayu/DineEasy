import { Check, Clock, ChefHat, Bell, UtensilsCrossed } from "lucide-react";
...
export function OrderStatusTracker({ status, className, compact = false }) {
          ...
          return (
            <div key={step.key} className="flex items-center">
              ...
              {index < steps.length - 1 && (
                <div className={cn("w-4 h-0.5 transition-all duration-300", index < currentIndex ? "bg-success" : "bg-muted")} />
              )}
            </div>
          );
        })}
              ...
              <div className="flex items-center w-full">
                {index > 0 && (
                  <motion.div
                    ...
                    className={cn("flex-1 h-1 origin-left transition-all duration-500", index <= currentIndex ? "bg-success" : "bg-muted")}
                  />
                )}
                <motion.div
...
export function OrderStatusBadge({ status }) {
  ...
  return <span className={cn("inline-flex items-center px-3 py-1 rounded-full text-xs font-medium border", config.className)}>{config.label}</span>;
}
