...
export function subscribeToOrdersBySession(sessionId, callback) {
  console.log(`[DataClient] Subscribing to orders for session: ${sessionId}`);
  // Remove orderBy to avoid index issues, sort in client
  const q = query(
    collection(db, "orders"), 
    where("sessionId", "==", sessionId)
  );
  return onSnapshot(q, (snapshot) => {
    console.log(`[DataClient] Received snapshot for session ${sessionId}, docs: ${snapshot.docs.length}`);
    const orders = snapshot.docs.map(doc => ({
      id: doc.id,
      ...doc.data(),
      createdAt: doc.data().createdAt?.toDate ? doc.data().createdAt.toDate().toISOString() : new Date().toISOString()
...
export async function updateOrderStatus(orderId, status) {
  console.log(`[DataClient] Updating order ${orderId} status to: ${status}`);
  try {
    ...
    console.log(`[DataClient] Order ${orderId} updated successfully`);
